...
router.get('/', authenticateJWT, function(req, res){
	let sql = "SELECT * FROM notification_mr WHERE mr_id = '"+req.body.id+"' AND status='0' ORDER BY id DESC LIMIT 10";
...
router.put('/', authenticateJWT, function(req, res){
	let sql = "UPDATE notification_mr SET status ='1' WHERE mr_id='"+req.body.id+"' ";
